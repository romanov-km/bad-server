...
export const getOrders = async (
    ...
    try {
        const safePage = (page: any) => {
            const parsedPage = Number(page);
            return parsedPage > 0 ? parsedPage : 1;
        };
        
        const safeLimit = (limit: any) => Math.min(10, Math.max(1, Number(limit) || 10))
        ...
        aggregatePipeline.push(
            { $sort: sort },
            { $skip: (Number(safePage(page)) - 1) * Number(safeLimit(limit)) }, // Ограничиваем
            { $limit: Number(safeLimit(limit)) }, // Ограничиваем
            {
                $group: {
                    _id: '$_id',
        ...
        const totalPages = Math.ceil(totalOrders / safeLimit(limit));
        res.status(200).json({
            orders,
            pagination: {
                totalOrders,
                totalPages,
                currentPage: Number(safePage(page)),
                pageSize: Number(safeLimit(limit)),
            },
        })
    } catch (error) {
        next(error)
